...
while True:
    ...
    #takip ettiğimiz beyaz alanlar siyah nesnelerdir
    ...
    for cnt in contours:
        area = cv2.contourArea(cnt) #alan hesaplaması yapılır
        if area > 500: #eşikleme yapılır
            x, y, w, h = cv2.boundingRect(cnt) #geometrik sınırlar bulunur
            cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2) #sınırlara dörtgen çizilir
